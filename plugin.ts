      ...
      () =>
        section.section({
          ...
          text: [
            ...
            'Five creative skills are registered into the dsh SkillRegistry (runtime layer, first-wins — never re-register or shadow them); load the matching skill via the skill tool when the chat intent fits:',
            '- cinema-dna-21x9x3: 电影三联 / 21:9 电影静帧 / 九宫格故事板 / 电影感分镜 (locks 21:9).',
            '- life-force-portrait: 游客照/废片升级 / 生命感人像 / 氛围感大片 (3:4, MODE A 保身份).',
            '- photography-simulation: 徕卡/哈苏/理光相机气质的街头或旅行摄影拼图 (3:4, 2-4 小图一组).',
            '- movie-poster: 9:16 中文电影海报 (片名+题材, 构成模型 10 选 1, 署名统一 梵想美学).',
            '- character-casting: 原创角色/选角图/人物设定 (character accuracy > beauty, NO WAX SKIN).',
            'Standard pipeline: istudio_skill_plan(skillId, brief) -> check selfCheck score/veto/passed -> istudio_generate(planId). Never invent tools outside the istudio_* family:',
            '- istudio_skill_plan: compile a skill brief into a CreativePlan (no images).',
            '- istudio_generate: text-to-image; pass planId from istudio_skill_plan when available; force:true bypasses the plan gate.',
            ...
            'Hard constraints: cinema-dna locks 21:9; poster mode is 3:4 and only when the user asks for 海报/封面/片名; when a plan is rejected (veto or score below threshold) istudio_generate fails with PLAN_REJECTED — explain the failures to the user and ask whether to revise the brief or retry with force:true.',
            'Red line: when a plan is vetoed or scores below threshold, istudio_generate rejects with PLAN_REJECTED. Explain the score/failures/veto to the user in Chinese and ask whether to revise the brief or force:true — never silently retry or force on your own.',
          ].join('\n'),
        }),
